package com.frank.filters.widget;
...
import com.frank.filters.camera.utils.CameraInfo;
import com.frank.filters.encoder.video.TextureMovieEncoder;
import com.frank.filters.filter.advanced.BeautyFilter;
import com.frank.filters.filter.base.CameraInputFilter;
...
public class FilterCameraView extends FilterBaseView {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        if (textureId == com.frank.filters.utils.OpenGlUtils.NO_TEXTURE) {
            textureId = com.frank.filters.utils.OpenGlUtils.getExternalOESTextureID();
            if (textureId != com.frank.filters.utils.OpenGlUtils.NO_TEXTURE) {
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (recordingEnabled) {
            switch (recordingStatus) {
                case RECORDING_OFF:
                    CameraInfo info = com.frank.filters.camera.CameraEngine.getCameraInfo();
    ...
    private void openCamera() {
        if (com.frank.filters.camera.CameraEngine.getCamera() == null) {
            com.frank.filters.camera.CameraEngine.openCamera();
        }
        CameraInfo info = com.frank.filters.camera.CameraEngine.getCameraInfo();
        ...
        if (surfaceTexture != null) {
            com.frank.filters.camera.CameraEngine.startPreview(surfaceTexture);
        }
    }
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        ...
        com.frank.filters.camera.CameraEngine.releaseCamera();
    }
    ...
    public void savePicture(final SavePictureTask savePictureTask) {
        com.frank.filters.camera.CameraEngine.takePicture(null, null, new Camera.PictureCallback() {
            @Override
            public void onPictureTaken(byte[] data, Camera camera) {
                com.frank.filters.camera.CameraEngine.stopPreview();
                ...
                queueEvent(new Runnable() {
                    ...
                    public void run() {
                        ...
                        final Bitmap photo = drawPhoto(bitmap, com.frank.filters.camera.CameraEngine.getCameraInfo().isFront);
                ...
                com.frank.filters.camera.CameraEngine.startPreview();
            }
        });
    }
    ...
    private Bitmap drawPhoto(Bitmap bitmap, boolean isRotated) {
        ...
        int textureId = com.frank.filters.utils.OpenGlUtils.loadTexture(bitmap, com.frank.filters.utils.OpenGlUtils.NO_TEXTURE, true);

        FloatBuffer gLCubeBuffer = ByteBuffer.allocateDirect(com.frank.filters.utils.TextureRotationUtil.CUBE.length * 4)
                .order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        FloatBuffer gLTextureBuffer = ByteBuffer.allocateDirect(com.frank.filters.utils.TextureRotationUtil.TEXTURE_NO_ROTATION.length * 4)
                .order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        gLCubeBuffer.put(com.frank.filters.utils.TextureRotationUtil.CUBE).position(0);
        if (isRotated) {
            gLTextureBuffer.put(com.frank.filters.utils.TextureRotationUtil.getRotation(com.frank.filters.utils.Rotation.NORMAL, false, false)).position(0);
        } else {
            gLTextureBuffer.put(com.frank.filters.utils.TextureRotationUtil.getRotation(com.frank.filters.utils.Rotation.NORMAL, false, true)).position(0);
        }
