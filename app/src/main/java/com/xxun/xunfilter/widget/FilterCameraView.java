package com.xxun.xunfilter.widget;
...
import com.xxun.xunfilter.camera.utils.CameraInfo;
import com.xxun.xunfilter.encoder.video.TextureMovieEncoder;
import com.xxun.xunfilter.filter.advanced.BeautyFilter;
import com.xxun.xunfilter.filter.base.CameraInputFilter;
...
public class FilterCameraView extends FilterBaseView {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        if (textureId == com.xxun.xunfilter.utils.OpenGlUtils.NO_TEXTURE) {
            textureId = com.xxun.xunfilter.utils.OpenGlUtils.getExternalOESTextureID();
            if (textureId != com.xxun.xunfilter.utils.OpenGlUtils.NO_TEXTURE) {
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (recordingEnabled) {
            switch (recordingStatus) {
                case RECORDING_OFF:
                    CameraInfo info = com.xxun.xunfilter.camera.CameraEngine.getCameraInfo();
    ...
    private void openCamera() {
        if (com.xxun.xunfilter.camera.CameraEngine.getCamera() == null) {
            com.xxun.xunfilter.camera.CameraEngine.openCamera();
        }
        CameraInfo info = com.xxun.xunfilter.camera.CameraEngine.getCameraInfo();
        ...
        if (surfaceTexture != null) {
            com.xxun.xunfilter.camera.CameraEngine.startPreview(surfaceTexture);
        }
    }
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        ...
        com.xxun.xunfilter.camera.CameraEngine.releaseCamera();
    }
    ...
    public void savePicture(final SavePictureTask savePictureTask) {
        com.xxun.xunfilter.camera.CameraEngine.takePicture(null, null, new Camera.PictureCallback() {
            @Override
            public void onPictureTaken(byte[] data, Camera camera) {
                ...
                com.xxun.xunfilter.camera.CameraEngine.startPreview();
            }
        });
    }
    ...
    private Bitmap drawPhoto(Bitmap bitmap, boolean isRotated) {
        ...
        int textureId = com.xxun.xunfilter.utils.OpenGlUtils.loadTexture(bitmap, com.xxun.xunfilter.utils.OpenGlUtils.NO_TEXTURE, true);

        FloatBuffer gLCubeBuffer = ByteBuffer.allocateDirect(com.xxun.xunfilter.utils.TextureRotationUtil.CUBE.length * 4)
                .order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        FloatBuffer gLTextureBuffer = ByteBuffer.allocateDirect(com.xxun.xunfilter.utils.TextureRotationUtil.TEXTURE_NO_ROTATION.length * 4)
                .order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        gLCubeBuffer.put(com.xxun.xunfilter.utils.TextureRotationUtil.CUBE).position(0);
        if (isRotated) {
            gLTextureBuffer.put(com.xxun.xunfilter.utils.TextureRotationUtil.getRotation(com.xxun.xunfilter.utils.Rotation.NORMAL, false, false)).position(0);
        } else {
            gLTextureBuffer.put(com.xxun.xunfilter.utils.TextureRotationUtil.getRotation(com.xxun.xunfilter.utils.Rotation.NORMAL, false, true)).position(0);
        }
