...
import com.xxun.camera.utils.CameraInfo;
import com.xxun.encoder.video.TextureMovieEncoder;
import com.xxun.filter.base.CameraInputFilter;
import com.xxun.filter.helper.FilterType;
import com.xxun.utils.SavePictureTask;
import com.xxun.utils.FilterParams;
...
//    private BeautyFilter beautyFilter;
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        if (textureId == com.xxun.utils.OpenGlUtils.NO_TEXTURE) {
            textureId = com.xxun.utils.OpenGlUtils.getExternalOESTextureID();
            if (textureId != com.xxun.utils.OpenGlUtils.NO_TEXTURE) {
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (recordingEnabled) {
            switch (recordingStatus) {
                case RECORDING_OFF:
                    CameraInfo info = com.xxun.camera.CameraEngine.getCameraInfo();
    ...
    private void openCamera() {
        if (com.xxun.camera.CameraEngine.getCamera() == null) {
            com.xxun.camera.CameraEngine.openCamera();
        }
        CameraInfo info = com.xxun.camera.CameraEngine.getCameraInfo();
        ...
        if (surfaceTexture != null) {
            com.xxun.camera.CameraEngine.startPreview(surfaceTexture);
        }
    }
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        ...
        com.xxun.camera.CameraEngine.releaseCamera();
    }
    ...
    public void savePicture(final SavePictureTask savePictureTask) {
        com.xxun.camera.CameraEngine.takePicture(null, null, new Camera.PictureCallback() {
            @Override
            public void onPictureTaken(byte[] data, Camera camera) {
                com.xxun.camera.CameraEngine.stopPreview();
                final Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0, data.length);
                queueEvent(new Runnable() {
                    @Override
                    public void run() {
                        final Bitmap photo = drawPhoto(bitmap, com.xxun.camera.CameraEngine.getCameraInfo().isFront);
                ...
                com.xxun.camera.CameraEngine.startPreview();
            }
        });
    }
...
//        beautyFilter.onInputSizeChanged(width, height);
        ...
        int textureId = com.xxun.utils.OpenGlUtils.loadTexture(bitmap, com.xxun.utils.OpenGlUtils.NO_TEXTURE, true);

        FloatBuffer gLCubeBuffer = ByteBuffer.allocateDirect(com.xxun.utils.TextureRotationUtil.CUBE.length * 4)
                .order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        FloatBuffer gLTextureBuffer = ByteBuffer.allocateDirect(com.xxun.utils.TextureRotationUtil.TEXTURE_NO_ROTATION.length * 4)
                .order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        gLCubeBuffer.put(com.xxun.utils.TextureRotationUtil.CUBE).position(0);
        if (isRotated) {
            gLTextureBuffer.put(com.xxun.utils.TextureRotationUtil.getRotation(com.xxun.utils.Rotation.NORMAL, false, false)).position(0);
        } else {
            gLTextureBuffer.put(com.xxun.utils.TextureRotationUtil.getRotation(com.xxun.utils.Rotation.NORMAL, false, true)).position(0);
        }
